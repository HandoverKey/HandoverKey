...
import { ShieldCheckIcon, LockOpenIcon, ExclamationTriangleIcon } from "@heroicons/react/24/outline";
...
const SuccessorAccess: React.FC = () => {
    ...
    const verifyToken = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                if (response.data.handoverStatus && allowedStatuses.includes(response.data.handoverStatus)) {
                    setStatus("VERIFIED");
                } else {
    ...
    const handleUnlockVault = async () => {
        ...
        try {
            ...
            showError("Failed to unlock vault. Please check if you have entered enough valid shares.");
        } finally {
            setUnlocking(false);
        }
    };
            ...
            <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
                <div className="sm:mx-auto sm:w-full sm:max-w-md">
                    <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 text-center">
                        ...
                        <h2 className="mt-6 text-2xl font-bold text-gray-900">Access Not Yet Available</h2>
                        <p className="mt-2 text-sm text-gray-600">
                            {metadata.userName}'s vault is not yet open for successors.
                            The handover process status is currently: <span className="font-semibold text-blue-600">{metadata.handoverStatus}</span>.
                        </p>
                        <p className="mt-4 text-xs text-gray-500 italic">
                            Access is only granted after the inactivity period and handover process has started.
                        </p>
                        <button
                            onClick={() => navigate("/")}
                            className="mt-6 btn btn-primary w-full"
                            ...
                            <div className="space-y-8">
                                    ...
                                    <div className="flex">
                                        ...
                                        <div className="ml-3">
                                            <p className="text-sm text-yellow-700">
                                                To unlock this vault, you need to combine your unique key share with shares from other successors.
                                                The total number of shares must meet the threshold set by the owner.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                                    <div className="sm:col-span-2">
                                        <label htmlFor="my-share" className="block text-sm font-medium text-gray-700">
                                            ...
                                            {peerShares.map((share, index) => (
                                                <div key={index} className="flex gap-2">
                                                    <textarea
                                                        ...
                                                        value={share}
                                                        onChange={(e) => handlePeerShareChange(index, e.target.value)}
                                                    />
                                                </div>
                                            ))}
                                        </div>
                            ...
                            <div className="space-y-6">
                                    ...
                                    <div className="flex">
                                        ...
                                        <div className="ml-3">
                                            <p className="text-sm text-green-700">
                                                Vault successfully unlocked. Decrypted data is shown below.
                                                <span className="font-bold"> Important:</span> This data is only available in your browser and is not saved anywhere.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                    {decryptedEntries.map((entry) => (
                                        <div key={entry.id} className="card p-6 border-l-4 border-blue-500">
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    ...
                                                    <h4 className="mt-2 text-lg font-bold text-gray-900">
                                                        {entry.decryptedData.title || entry.decryptedData.name || "Untitled Entry"}
                                                    </h4>
                                                </div>
                                            </div>
